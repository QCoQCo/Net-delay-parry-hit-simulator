// Wraps a raw send function with artificial latency, jitter and packet
// loss. Used identically on top of the loopback transport and the real
// WebSocket transport, so the sliders behave the same way in both modes.
export class NetworkImpairment {
  ...
  send(packet) {
    if (Math.random() * 100 < this.lossPercent) return; // dropped
