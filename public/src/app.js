...
function statusFor(runner) {
    ...
    const base =
        performance.now() - runner.lastRollbackTime < ROLLBACK_INDICATOR_MS
            ? {
                  status: `⟲ 롤백: ${runner.lastRollbackFrames}프레임 재시뮬레이션`,
                  statusColor: '#e8c04a',
              }
            : { status: '예측 진행 중', statusColor: '#8b95a3' };
    if (runner.desyncFrame !== null) {
        base.sync = `⚠ DESYNC · frame ${runner.desyncFrame}에서 상태 불일치 확인`;
        base.syncColor = '#ff5a5a';
    } else if (runner.lastSyncedFrame >= 0) {
        base.sync = `sync ✓ f${runner.lastSyncedFrame}`;
        base.syncColor = '#5a8f5f';
    }
    return base;
}
