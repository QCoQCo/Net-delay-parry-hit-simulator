// Canvas rendering for the 1D parry/hit stage. Pure drawing code - takes
// a GameState and paints it, no simulation logic here.
...
const STATE_COLORS = {
  idle: null, // uses the player's own color
  startup: '#e8c04a',
  active: '#ff5a5a',
  recovery: '#5a6472',
  hitstun: '#ffffff',
...
export function renderStage(ctx, canvas, state, hud) {
  ...
  state.players.forEach((player, i) => {
    // Active-frame hit range indicator
  ...
  if (hud.sync) {
    ctx.textAlign = 'right';
    ctx.fillStyle = hud.syncColor ?? '#8b95a3';
    ctx.fillText(hud.sync, width - 14, height - 10);
  }
}
